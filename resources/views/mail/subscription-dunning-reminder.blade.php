...
@section('badge', $isFinal ? 'Final notice' : 'Payment reminder')
...
@section('content')
    ...
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;color:#5b6472;">
        @if ($isFinal)
            This is our final reminder: the renewal payment for your {{ $subscription->product->name }} subscription is still outstanding.
            @if ($daysUntilSuspension !== null)
                Unless payment is completed within {{ $daysUntilSuspension }} {{ \Illuminate\Support\Str::plural('day', $daysUntilSuspension) }}, your access will be suspended.
            @endif
        @else
            This is a friendly reminder that the renewal payment for your {{ $subscription->product->name }} subscription is still outstanding.
        @endif
    </p>
    ...
    <p style="margin:24px 0 0;font-size:13px;line-height:1.7;color:#5b6472;">
        Sign in to the client area and choose Update payment method to keep your access uninterrupted.
    </p>
@endsection
